...
def init_db():
    ...
    # check_same_thread=False is REQUIRED for a multi-threaded
    # server that shares one connection object.
    ...
    try:
        ...
        # Also create an index on timestamp for fast time-based queries
...
def insert_log(conn, log_data):
    ...
    # Get the keys and values from our log
    # We use .get() to avoid errors if a key is missing
    keys = []
    values = []
    
    # Dynamically build the keys and values list
    # Start with 'timestamp' as it's a required column
